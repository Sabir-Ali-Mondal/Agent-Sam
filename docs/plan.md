...
LAUNCH AGENT & SYSTEM INITIALIZATION
        ...
        ├─ Initialize Flexible Action Parser (JSON + Tags) & Loop detector
        ├─ Initialize user input monitor
        ├─ Initialize AI Cursor Overlay (Main Thread UI)
        └─ Initialize system tray + dashboard
        │
        ▼
LOAD MEMORY STRUCTURES
        │
        ├─ Short-term memory (current goal + history)
...
CONTEXT BUILDER & RAG MEMORY INJECTION
        │
        ├─ User goal & Current subgoal (Goal State)
        ...
        ├─ Cursor position & Last actions (History)
        └─ Context Size Limiter (Top 20 elements, 5-action history limit)
        │
        ▼
PROMPT BUILDER & AI REASONING
        │
        ├─ Insert system prompt & Flexible Action Schema (JSON + Tags)
...
RECEIVE RESPONSE & CLASSIFICATION
        ...
        │
        ▼
FLEXIBLE ACTION PARSER & LOOP DETECTOR
        │
        ├─ Valid JSON or Tag Format? (If No -> Attempt repair -> Request regeneration)
...
**Context Serialization Format (Simplified for TAG format)**
*Security Note: Clipboard contents are excluded. All operations are blind.*

The simplified structure injected into the AI context:
```json
{
  ...
  "previous_actions":[
    {"type": "gui_interaction", "action": "click", "target": "A12"}
  ]
}
```
...
> Rules:
> 1. Output ONLY JSON or the @tag format. Do not include markdown code fences (```) or conversational text.
> 2. Classify your action as 'direct_execution', 'gui_interaction', or 'interactive_request'.
> 3. Prefer Direct System Execution for file, app, and web management.
> 4. Never hallucinate elements.
> 5. If using GUI and the target is not visible, use `scroll_down`.
> 6. Use 'interactive_request' for sensitive data or destructive actions.
> 7. If the last GUI action did NOT change the screen, do NOT repeat it; choose an alternative action.
> 8. Stop when the goal is achieved."

**AI Action Schema (Flexible Formats)**

The agent primarily supports the **Tag Format** for maximum speed and reliability, but can fall back to JSON repair.

**Production Tag Format (Recommended)**
```text
@thought: Step-by-step reasoning
@goal: The overall objective
@type: gui_interaction | direct_execution | interactive_request | system
@action: click | open_app | web_search | etc.
@name: App name or Search query
@url: URL
@path: File path
@text: Text to type or command to run
@keys: alt+tab (modifiers with +, sequential with ,)
@target: A01 (Grid location or element ID)
@x: 100
@y: 200
```
...
**Flexible Action Parsing Layer**
If the AI output is not valid JSON, the system attempts a two-stage recovery:
1.  **Tag-based Extraction:** Checks for the presence of `@action`, `@tile`, etc. This format is inherently more robust for small models that struggle with JSON brackets.
2.  **JSON Repair:** If Tags are missing but JSON-like text exists, it uses regex and balanced-bracket matching to extract and repair the first valid JSON object found in the response.

**Search-First Execution Philosophy**
The agent uses **Windows Search** and **Web Search** as its primary execution engine to access applications, files, and information. This approach leverages the system's built-in NLP and resolution capabilities to verify existence and launch operations reliably.

1.  **Windows Search (Local):** Used for launching apps (e.g., searching "notepad"), finding files, and system settings.
2.  **Web Search (Global):** Used for information retrieval or finding web-based services.
3.  **Direct Execution (Smart):** Commands like `open_app` utilize shell-level search to resolve names dynamically.

**AI Cursor Overlay & Dynamic Token Management**
*   **Secondary AI Cursor:** A visual, transparent, and click-through overlay (green/red tricone) is rendered at the AI's target coordinates. This allows the user to see exactly where the AI intends to click without moving the actual hardware cursor until the execution step.
*   **Dynamic Max Tokens:** The `max_tokens` parameter for both local and cloud inference is calculated dynamically as **2x the total prompt size** (text + image tokens). This ensures the AI has sufficient generation room for complex reasoning and structured outputs while preventing 400 Bad Request errors from exceeding context limits.
